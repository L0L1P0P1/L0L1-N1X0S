...

{
  ...
  config = lib.mkIf config.picom.enable {
    ...
    systemd.user.services.picom.Service.ExecStart =
      let
        picomConf = pkgsUnstable.writeText "picom.conf" ''
          backend = "glx";
          vsync = false;
          shadow = true;
          fading = true;
          ...
          corner-radius = ${toString config.picom.cornerRadius};
        ...
        '';
      in
      lib.mkForce "${pkgsUnstable.picom}/bin/picom --config ${picomConf} --log-level error";
  };
}
